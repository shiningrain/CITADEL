...
convs = [func_cls(in_channels, out_channels, kernel_size=3, bias=False).to(device) for _ in range(num_convs)]

fused_conv = func_cls(in_channels=convs[0].in_channels * num_convs, out_channels=convs[0].out_channels * num_convs, groups=convs[0].groups * num_convs, kernel_size=convs[0].kernel_size, bias=convs[0].bias).to(device)
