...
def load_similarity_score(similarity_path,threshold):
    ...
    path_list=[f'./result/score_sys_{threshold}.pkl',
    f'./result/pairs_sys_{threshold}.pkl',
    './result/score_deeprel.pkl',
    './result/pairs_deeprel.pkl']
    if os.path.exists(path_list[0]):
        return path_list

    if os.path.exists('./result/score_deeprel.pkl'):
        with open('./result/score_deeprel.pkl', 'rb') as f:#input,bug type,params
    ...
    for key in score_dict.keys():
        ...
        if record and 'argument' in score_dict[key].keys() and score_dict[key]['argument']>0.6:
            tmp=key.split('-')
            tmp.sort()
...
def extract_similarity(save_dir,threshold=60):
    # score_list=function_relation_dict['similarity score']
...
def remove_useless_operation(relation_dict,threshold=30):
    ...
    # plt.ylim(ymax=100)
    # plt.savefig('./result/tmp-0404.pdf')
    return relation_dict
...
def new_get_raw_similarity(save_dir,threshold):
...
def verify(threshold, path_list):
    ...
    deeprel_verify_path=path_list[3].replace('pairs','verify')
    sys_verify_path=path_list[1].replace('pairs','verify')
    ...
    # sys_pair={'torch.nn.Conv2d':sys_pair['torch.nn.Conv2d']}#['torch.nn.LazyConv2d']}
...
if __name__=='__main__':
    ...
    # 2. use threshold to get all match APIs, get the path list that save the match results
    path_list=new_get_raw_similarity(save_dir,threshold=value)
    # 3. use arugment to verify the similarity functions
    new_combine_dict=verify(value,path_list)
    
    # 4. save all matched API pairs
    with open(f'./result/1_final_relation.pkl', 'wb') as f:
        # We saved all matched APIs in this dict, the key is function and value is the list of its similar APIs.
        pickle.dump(new_combine_dict['total_pair'], f)
    # with open(f'./result/1_all_matched_results.pkl', 'wb') as f:
