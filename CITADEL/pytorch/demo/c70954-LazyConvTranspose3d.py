...
def build_model(ch: int, groups: int = 1) -> nn.Module:
    return nn.Sequential(
        func_cls(out_channels=ch, kernel_size=(1, 1, 1)),
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 0
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 1
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 2
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 3
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 4
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 5
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 6
        func_cls(out_channels=ch, kernel_size=(3, 3, 3), padding=1, groups=groups),  # 7
...
def perf_test(ch: int, groups: int, dtype) -> float:
    model = build_model(ch, groups).to(dtype)
            images=torch.from_numpy(np.full((batch_size,3,input_w,input_w,input_w),0.5)).to(dtype)
