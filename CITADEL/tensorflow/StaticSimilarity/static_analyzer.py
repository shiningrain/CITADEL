...
def save_function(filename,program_list):
    ...
    for l in range(len(text_list)):
        ...
        if blank_num==0 and tmp_list!=[]:
            ...
            # if 'class' in tmp_list[0] and '{\n' in tmp_list[0]:
            #     # output_list.append(indent_list[l]+text_list[l])
            #     tmp_list=[]
            ...
            tmp_list=[]
    if program_in_file!=[]:
        program_list.append((filename.replace('/home/zxy/main/DL_compiler_test/1_test_code/0731_tf_test/tf_build/tf/tensorflow/tensorflow/', '/tensorflow/'),program_in_file))
    return program_list

def file_name(file_dir,program_list,file_type=['cpp','cc']):# torch cpp, tf cc
...
def extract_msg(program_list):
    ...
    for pl in program_list:
        key=pl[0].replace('/home/zxy/main/DL_compiler_test/1_test_code/0731_tf_test/tf_build/tf/tensorflow/tensorflow/', '/tensorflow/').split('.')[0]
        function_list=pl[1]
        # program_dict[key]={}
...
def extract_similar_get_all(program_dict):
    ...
    filename = open('./c++_index-4dirs.txt', 'w')
    # for tmp_string in function_name_list:
    filename.writelines([tmp+'\n' for tmp in function_name_list])
    filename.close()
    ...
    for tup in generate_tuples(function_name_list):
        ...
        if (input_score>0.8) and (call_score>0.8 and call_score<1.0001): # update here
            pair_list.append([tup,[input_score,call_score]])
        
            if len(pair_list)%100==0:
                with open('./tmp/c++_new_pair.pkl', 'wb') as f:
...
def process_new_similar(pair_list):
    new_pair_list=[]
    for pl in pair_list:
        if (pl[1][0]>0.8) and (pl[1][-1]>0.8):
            new_pair_list.append(pl)
    
    
    new_list=[]
    for l in trange(len(new_pair_list)):
        pl=new_pair_list[l]
        similar_function_list=pl[0]
        if new_list==[]:
...
if __name__=='__main__':
    program_list=[]
    save_path='./tmp_data-4dirs.pkl'
    program_path='./program_msg-4dirs.pkl'
    save_path_1='./c++_new_pair-4dirs.pkl'
    csv_path='./new_similar_pair-4dirs.csv'
    simi_path='./new_similar_pair-4dirs.pkl'
    index_path='./c++_index-4dirs.txt'
    ...
    f.close()
    index_list=[index_list[i].replace('\n', '') for i in range(len(index_list))]
