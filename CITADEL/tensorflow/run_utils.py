...
class Code_Processor:
    ...
    def add_import(self,codes):
        ...
        for line in codes:
            ...
            if add_sign and not finish_sign:
                ...
                new_codes.append("sys.path.append('../../tf_code')\n")
    ...
    def modify_function(self,codes,func_name):
        ...
        pattern=f'[a-zA-Z1-9_.]*({func_name})'
...
def generate_codes(func_list,code_path,input_process=False,contain_functions=None):
    simi_func_list=[]
    # framework='other'
    for i in range(len(func_list)):
        simi_func_list.append(string2function(func_list[i]))
        # if 'tf.keras.' in func_list[i]:# tf.keras dont has usable signatures
        #     framework='tf-keras'
    code_path_dict=code_process(simi_func_list[0],simi_func_list[1:],code_path,method='run',func_list=func_list)
    # framework=framework,func_list=func_list,contain_functions=contain_functions
    # some tf functions have no signature, which cause our code argument and dimension processer failed
    if input_process:
...
def execute_codes(code_path,origin_func_list,func_list,version,method,input_process=False,contain_functions=None):
    '''
    # method: p: time; s: exception; v:value
    ...
    code_path_dict=generate_codes(func_list,code_path,input_process=input_process,contain_functions=contain_functions)# generate codes for some similar functions if neccessary
    # if code_path_dict=={}:
    #     return []
    ...
    for i in range(len(func_list)):
        ...
        if i==0:
            ...
            elif func.split('.')[-1] in code_path_dict.keys():
                run_code_path=code_path_dict[func.split('.')[-1]][0]
            elif func in code_path_dict.keys():
                run_code_path=code_path_dict[func][0]
            else:
                run_code_path=code_path
        
        code_name=os.path.basename(run_code_path)
        use_docker=True
        if version==13 or version==14:
            ...
            command=f"docker exec -it {docker_dict['DOCKER_11']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==10:
            ...
            command=f"docker exec -it {docker_dict['DOCKER_6']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==5:
            command=f"docker exec -it {docker_dict['DOCKER_5']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==4:
            command=f"docker exec -it {docker_dict['DOCKER_4']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==3:
            command=f"docker exec -it {docker_dict['DOCKER_3']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==2:
            command=f"docker exec -it {docker_dict['DOCKER_2']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        elif version==1:
            command=f"docker exec -it {docker_dict['DOCKER_1']} bash -c 'cd {docker_dir_name} ; python {code_name} {func}'"
        else:
            command=f'cd {dir_name}; python {code_name} {func}'
            use_docker=False
            # default: run in current envs
        
        output_value=None
        
        if method=='p' and i==0:
            output=subprocess.run(command, timeout=120, shell=True,capture_output=True)# warm up

        time0=time.time()
        try:
            output=subprocess.run(command, timeout=120, shell=True,capture_output=True)
        except:
            if i==0:
                return []
            # else:
        ...
        if error_message!="b''":
            ...
            if 'Traceback (most recent call last):' in error_message:
                status=error_message.split('Traceback (most recent call last):')[-1]
            elif 'Error:' in error_message:
                status=error_message.split('Error:')[-1]
            elif 'tensorflow/core/' in error_message:
                status=error_message.split('tensorflow/core/')[-1]
            elif 'tensorflow/compiler/' in error_message:
                status=error_message.split('tensorflow/compiler/')[-1]
        ...
        if method=='s':
            if i==0 and status=='None':
                print('no status bug for original case, cannot reproduce!')
                return []
            ...
            elif isinstance(output_value,TSF.Tensor):
                output_value=output_value.cpu().numpy()
            ...
            or isinstance(output_value, np.ndarray) or isinstance(output_value, str):
                ...
                if output_value==None:
                    if i==0:
                        print('no value bug and cannot find output value! cannot reproduce')
                        return []
                    output_value=status
